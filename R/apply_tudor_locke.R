...
#'
#' The Tudor-Locke algorithm detects periods of time in bed and, for each
#' period, computes sleep quality metrics such as total minutes in bed, total
#' sleep time, number and average length of awakenings, movement and
#' fragmentation index.
#' @param agdb A \code{tibble} (\code{tbl}) of activity data (at least) an
#' \code{epochlength} attribute. The epoch length must be 60 seconds. Each
#' epoch should be scored as asleep (S) or awake (W), using the Sadeh,
...
#' @param min_nonzero_epochs Minimum number of epochs with non-zero activity.
#' The default is 0.
#' @return A summary \code{tibble} of the detected sleep periods. If the
#' activity data is grouped, then sleep periods are detected separately for
#' each group.
#' \describe{
...
#'   \item{efficiency}{The number of sleep minutes divided by the bedtime
#'   minutes.}
#'   \item{duration}{The duration of the sleep period, in minutes.}
#'   \item{activity_counts}{The sum of activity counts for the entire sleep
#'   period.}
#'   \item{total_sleep_time}{The number of minutes scored as "asleep" during
#'   the sleep period.}
...
#'   \item{ave_awakening}{The average length, in minutes, of all awakening
#'   episodes.}
#'   \item{movement_index}{Proportion of awake time out of the total time in
#'   bed, in percentages.}
#'   \item{fragmentation_index}{Proportion of one-minute sleep bouts out of
#'   the number of sleep bouts of any length, in percentages.}
...
#' use the Tudor-Locke algorithm to detect periods of \emph{bedtime} and
#' \emph{sleep time}. By definition, sleep time < bedtime since one can be
#' in bed and not sleeping.
#'
#' Bedtime is (the first minute of) \code{n_bedtime_start} consecutive
#' epochs/minutes labeled asleep (S). Similarly, wake time is (the first
#' minute of) of \code{n_wake_time_end} consecutive epochs/minutes labeled
#' awake (W), after a period of sleep. The block of time between bedtime
#' and wake time is one sleep period, if the time elapsed is at least
#' \code{min_sleep_period} minutes. There can be multiple sleep periods in
#' 24 hours but a sleep period cannot be longer than \code{max_sleep_period}
#' minutes.
#'
#' For each sleep period, the algorithm calculates several measures of sleep
#' quality such as time asleep and time awake, number and average length of
#' awakenings, and movement and fragmentation indices.
#'
...
#' }
#' @references C Tudor-Locke, TV Barreira, JM Schuna Jr, EF Mire and PT
#' Katzmarzyk. Fully automated waist-worn accelerometer algorithm for
#' detecting children's sleep-period time separate from 24-h physical
#' activity or sedentary behaviors. \emph{Applied Physiology}, Nutrition,
#' and Metabolism, 39(1):53–57, 2014.
