...
The `actigraph.sleepr` package implements three sleep detection algorithms: Sadeh (Sadeh, Sharkey, and Carskadon 1994), Cole-Kripke (Cole et al. 1992) and Tudor-Locke (Tudor-Locke et al. 2014). For illustration, let's use GT3X+ / ActiSleep+ sample data taken from ActiGraph's [online documentation](https://actigraph.desk.com).
...
Alternatively, we can use the `read_agd` function to get the raw activity measurements in a more convenient format: a `dplyr` data frame (a tibble) of timestamped activity counts, whose attributes are the device settings.
...
ggplot(agdb_10s, aes(timestamp, axis1)) +
  geom_line() +
  labs(x = "", y = "movement (vertical direction)", subtitle = 
         "24 hours of activity measured every 10 seconds") +
...
plot_activity(agdb_60s, "axis1") +
  labs(x = "", y = "movement (vertical direction)", subtitle = 
         "24 hours of activity data measured every minute") +
  scale_x_datetime(date_labels = "%I%p")
```
...
read_collapse_write <- function(filename, path_in, path_out) {
...
list.files(path_in, pattern = ".agd", full.names = FALSE) %>%
  map(read_collapse_write, path_in, path_out)
```
...
The time series of activity counts is padded with zeros as necessary, at the beginning and at the end, to compute the three functions AVG, SD, NATS within a rolling window. Finally, the sleep state is awake (1) if the sleep index SI is greater than -4; otherwise the sleep state is asleep (0).

```{r }
ggplot(agdb_sadeh, aes(timestamp, count)) +
  geom_line() +
  geom_point(aes(timestamp, 300 * sleep, 
                 color = as.factor(sleep)), shape = 1) +
  labs(x = "", y = "movement (bounded at 300)", subtitle =
         "For each epoch, Sadeh infers whether the subject is asleep or not") +
  scale_x_datetime(date_labels = "%I%p") +
...
ggplot(agdb_colekripke, aes(timestamp, count)) +
  ...
  geom_point(aes(timestamp, 100 * sleep, 
                 color = as.factor(sleep)), shape = 1) +
  labs(x = "", y = "movement (scaled by 0.01)", subtitle =
         "Like Sadeh, Cole-Kripke infers whether the subject is asleep or not") +
  scale_x_datetime(date_labels = "%I%p") +
  scale_color_discrete(name = "", labels = c("asleep (0)", "awake (1)"))
```
...
```{r }
plot_activity_period(agdb_60s, periods_sleep, "axis1", 
                     "in_bed_time", "out_bed_time", 
                     fill = "#AAAAAA") +
  scale_x_datetime(date_labels = "%I%p") +
  labs(x = "", y = "movement", subtitle = 
...
```{r }
periods_awake <- complement_periods(periods_sleep, agdb_sadeh,
...
agdb_awake <- combine_epochs_periods(agdb_sadeh, periods_awake,
                                     start_var = "period_start", 
                                     end_var = "period_end")
agdb_awake
