...
plot_activity(agdb_sadeh, axis1, color = "sleep") +
  labs(
    ...
    title = "For each epoch, Sadeh infers whether the subject is asleep (red) or awake (blue)"
  ) +
  scale_x_datetime(date_labels = "%I%p") +
...
plot_activity(agdb_colekripke, axis1, color = "sleep") +
  labs(
    x = "",
    y = "movement (bounded at 300)",
    title = "For each epoch, Cole-Kripke infers whether the subject is asleep (red) or awake (blue)"
  ) +
  scale_x_datetime(date_labels = "%I%p") +
  guides(color = FALSE, fill = FALSE)
```
...
plot_activity_period(
  ...
  labs(
    ...
    y = "movement",
    title = paste0(
      "Tudor-Locke detects sleep periods in a series of sleep-scored epochs (Ws and Ss)\n",
