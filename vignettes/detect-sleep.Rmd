...
ggplot(agdb_sadeh, aes(timestamp, count)) +
  ...
  labs(x = "", y = "activity (bounded at 300)", subtitle =
...
x <- c(rep(periods_sleep$in_bed_time, times = 2),
       rep(periods_sleep$out_bed_time, times = 2))
...
Let's combine the epoch counts data and the sleep periods data. Then we can easily slice the epochs data by sleep/non-sleep periods.

```{r }
# Let's label the epochs with a `period_id`, which indicates the sleep period
