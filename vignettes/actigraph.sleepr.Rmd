...
ggplot(agdb_colekripke, aes(timestamp, count)) +
  ...
  scale_x_datetime(date_labels = "%H:%M") +
  scale_color_discrete(name = "", labels = c("asleep (S)", "awake (W)"))
```
...
ggplot(sleep_periods, aes(x, y)) + 
  geom_polygon(aes(group = id), fill = "#DDDDDD", alpha = 0.5) +
  geom_line(data = agdb_60s, aes(timestamp, axis1)) +
  scale_x_datetime(date_labels = "%H:%M") +
  labs(x = "", y = "activity",subtitle = 
         paste0("Tudor-Locke detects sleep periods in a sequence of S's and W's\n",
