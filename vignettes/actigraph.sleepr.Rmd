...
The `actigraph.sleepr` package implements functions to read AGD files and to apply three standard sleep algorithms: Sadeh, Cole-Kripke and Tudor-Locke. For illustration, GT3X+ / ActiSleep+ sample data is taken from [ActiGraph's online documentation](https://actigraph.desk.com).
...
```{r fig.width = 7, fig.height = 4, fig.cap = "For each one-minute epoch, the Sadeh algorithm infers whether the subject is sleeping or not. A blue dot means awake (W), a red dot means asleep (S). The algorithm thresholds the raw activity counts at 300."}
...
```{r fig.width = 7, fig.height = 4, fig.cap = "For each one-minute epoch, the Cole-Kripke algorithm infers whether the subject is sleeping or not. A blue dot means awake (W), a red dot means asleep (S)."}
...
```{r fig.width = 7, fig.height = 4, fig.cap = "Given the asleep/awake state of each epoch, the Tudor-Locke algorithm detects sleep periods. These are highlighted as gray rectangles."}
